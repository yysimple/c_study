...
int dns_pack_request(unsigned char *buf, int *size, const char *name) {
    int maxsize = *size;
    if (maxsize < sizeof(struct dns_header_t)) return 1;
    int pos = 0;

    struct dns_header_t header;
    memset(&header, 0, sizeof(struct dns_header_t));
    header.tid = htons(gen_tid());
    header.flags = htons(0x0100);           // Query, Standard Mode, Recursion Desired
    header.qdcount = htons(1);
    memcpy(buf + pos, &header, sizeof(struct dns_header_t));
    pos += sizeof(header);
...
int dns_parse_response(unsigned char *buf, int size, struct ipaddr_t **addrs, int *addrnum) {
    if (size < sizeof(struct dns_header_t)) return 1;
    int pos = 0;

    struct dns_header_t *header = (struct dns_header_t *) (buf + pos);
    pos += sizeof(struct dns_header_t);
    ...
    struct ipaddr_t *retads = malloc(sizeof(struct ipaddr_t) * ancount);
